...
import org.floj.protocols.channels.PaymentChannelClient;
import org.floj.protocols.channels.PaymentChannelClientState;
import org.floj.protocols.channels.PaymentChannelServerState;
import org.floj.protocols.channels.PaymentChannelV1ClientState;
import org.floj.protocols.channels.PaymentChannelV1ServerState;
import org.floj.protocols.channels.PaymentChannelV2ClientState;
import org.floj.protocols.channels.PaymentChannelV2ServerState;
import org.floj.protocols.channels.StoredPaymentChannelClientStates;
import org.floj.protocols.channels.ValueOutOfRangeException;
import org.floj.script.Script;
import org.floj.script.ScriptBuilder;
import org.floj.testing.TestWithWallet;
...
import static org.floj.testing.FakeTxBuilder.createFakeTx;
import static org.floj.testing.FakeTxBuilder.makeSolvedTestBlock;
...
public class PaymentChannelStateTest extends TestWithWallet {
    ...
    public void basic() throws Exception {
        ...
        Transaction refund;
        if (useRefunds()) {
            refund = new Transaction(PARAMS, clientV1State().getIncompleteRefundTransaction().floSerialize());
        ...
        Transaction multisigContract = new Transaction(PARAMS, clientState.getContract().floSerialize());
        ...
        for (int i = 0; i < 4; i++) {
            byte[] signature = clientState.incrementPaymentBy(size, null).signature.encodeToFLO();
        ...
        final Transaction reserializedCloseTx = new Transaction(PARAMS, closeTx.floSerialize());
    ...
    public void setupDoS() throws Exception {
        ...
        if (useRefunds()) {
            // Send the refund tx from client to server and get back the signature.
            Transaction refund = new Transaction(PARAMS, clientV1State().getIncompleteRefundTransaction().floSerialize());
            byte[] refundSig = serverV1State().provideRefundTransaction(refund, myKey.getPubKey());
            assertEquals(PaymentChannelServerState.State.WAITING_FOR_MULTISIG_CONTRACT, serverState.getState());
            // This verifies that the refund can spend the multi-sig output when run.
            clientV1State().provideRefundSignature(refundSig, null);
        ...
        serverState.incrementPayment(CENT.divide(2).subtract(CENT.divide(10)),
                clientState.incrementPaymentBy(CENT.divide(10), null).signature.encodeToFLO());
    ...
    public void checkBadData() throws Exception {
        ...
        if (useRefunds()) {
            ...
            byte[] refundTxBytes = clientV1State().getIncompleteRefundTransaction().floSerialize();
        ...
        byte[] multisigContractSerialized = clientState.getContract().floSerialize();
        ...
        byte[] signature2 = clientState.incrementPaymentBy(HALF_COIN.subtract(totalPayment), null).signature.encodeToFLO();
    ...
    public void feesTest() throws Exception {
        ...
        byte[] signature = clientState.incrementPaymentBy(Coin.SATOSHI, null).signature.encodeToFLO();
        ...
        serverState.incrementPayment(CENT.subtract(totalPayment), payment.signature.encodeToFLO());
    ...
    public void serverAddsFeeTest() throws Exception {
        ...
        signature = clientState.incrementPaymentBy(SATOSHI, null).signature.encodeToFLO();
    ...
    public void doubleSpendContractTest() throws Exception {
        ...
        doubleSpendContract = new Transaction(PARAMS, doubleSpendContract.floSerialize());
