...
import org.floj.core.ProtocolException;
...
import static org.floj.script.ScriptOpCodes.*;
 ...
 *    rethink your life. Seriously, why are you reimplementing FLO consensus rules? Instead, go work on making
 *    FLO Core consensus rules a shared library and use that. Seriously, you wont get it right, and starting with
 *    this tester as a way to try to do so will simply end in pain and lost coins. SERIOUSLY, JUST STOP!
 *
 * b) FLO Core is faling some test in here and you're wondering what test is causing failure. Just stop. There is no
...
public class FullBlockTestGenerator {
    // Used by FLOdComparisonTool and AbstractFullPrunedBlockChainTest to create test cases
    ...
    public RuleList getBlocksToTest(boolean runBarelyExpensiveTests, boolean runExpensiveTests, File blockStorageFile) throws ScriptException, ProtocolException, IOException {
        ...
        spendableOutputs.offer(new TransactionOutPointWithValue(
                ...
                HUNDRED_COINS, chainHead.getTransactions().get(0).getOutputs().get(0).getScriptPubKey()));
    ...
    private NewBlock createNextBlock(Block baseBlock, int nextBlockHeight, @Nullable TransactionOutPointWithValue prevOut,
        ...
        Coin coinbaseValue = HUNDRED_COINS.shiftRight(nextBlockHeight / params.getSubsidyDecreaseBlockCount())
