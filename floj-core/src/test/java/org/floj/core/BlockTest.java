...
import org.floj.core.AbstractBlockChain.NewBlockType;
import org.floj.params.MainNetParams;
import org.floj.params.TestNet2Params;
...
public class BlockTest {
    ...
    public void testWork() throws Exception {
        ...
        // This number is printed by FLO Core at startup as the calculated value of chainWork on testnet:
        //
    ...
    @Test
    public void testFLOSerialization() throws Exception {
    ...
    public void testUpdateLength() {
        ...
        assertEquals(tx.unsafeFLOSerialize().length, tx.length);
        assertEquals(origTxLength, tx.length);
        block.addTransaction(tx);
        assertEquals(block.unsafeFLOSerialize().length, block.length);
        ...
        assertEquals(block.length, block.unsafeFLOSerialize().length);
        assertEquals(block.length, origBlockLen + tx.length);
        assertEquals(tx.length, origTxLength - 1);
