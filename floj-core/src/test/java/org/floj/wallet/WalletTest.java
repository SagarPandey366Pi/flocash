...
import org.floj.core.TransactionConfidence.ConfidenceType;
import org.floj.core.listeners.TransactionConfidenceEventListener;
import org.floj.crypto.*;
import org.floj.script.Script;
import org.floj.script.ScriptBuilder;
import org.floj.signers.StatelessTransactionSigner;
import org.floj.signers.TransactionSigner;
import org.floj.store.BlockStoreException;
import org.floj.store.MemoryBlockStore;
import org.floj.testing.*;
import org.floj.utils.ExchangeRate;
import org.floj.utils.Fiat;
import org.floj.utils.Threading;
import org.floj.wallet.AllowUnconfirmedCoinSelector;
import org.floj.wallet.CoinSelector;
import org.floj.wallet.DefaultCoinSelector;
import org.floj.wallet.DeterministicKeyChain;
import org.floj.wallet.DeterministicUpgradeRequiresPassword;
import org.floj.wallet.KeyBag;
import org.floj.wallet.KeyChain;
import org.floj.wallet.KeyChainGroup;
import org.floj.wallet.KeyTimeCoinSelector;
import org.floj.wallet.MarriedKeyChain;
import org.floj.wallet.Protos;
import org.floj.wallet.RiskAnalysis;
import org.floj.wallet.SendRequest;
import org.floj.wallet.UnreadableWalletException;
import org.floj.wallet.Wallet;
import org.floj.wallet.WalletFiles;
import org.floj.wallet.WalletProtobufSerializer;
import org.floj.wallet.WalletTransaction;
import org.floj.wallet.Protos.Wallet.EncryptionType;
import org.floj.wallet.Wallet.BalanceType;
import org.floj.wallet.WalletTransaction.Pool;
...
import static org.floj.testing.FakeTxBuilder.*;
...
public class WalletTest extends TestWithWallet {
    ...
    private void basicSpendingCommon(Wallet wallet, Address toAddress, Address destination, Wallet encryptedWallet) throws Exception {
        ...
        // will attach a small fee. Because the FLO protocol makes it difficult to determine the fee of an
    ...
    // identical result to FLO Core. However the signatures are not deterministic - signing the same data
    ...
    public void blockChainCatchup() throws Exception {
        ...
        // What we'd really like to do is prove FLO Core would accept it .... no such luck unfortunately.
    ...
    public void spendToSameWallet() throws Exception {
        ...
        assertEquals(ZERO, wallet.getBalance());
        sendMoneyToWallet(AbstractBlockChain.NewBlockType.BEST_CHAIN, outbound1);
        // We should have a balance of 1 FLO after the block is received.
        assertEquals(coin1, wallet.getBalance());
    }
    ...
    public void watchingScriptsBloomFilter() throws Exception {
        ...
        assertFalse(wallet.getBloomFilter(1e-12).contains(outPoint.unsafeFLOSerialize()));

        sendMoneyToWallet(BlockChain.NewBlockType.BEST_CHAIN, t1);
        assertTrue(wallet.getBloomFilter(1e-12).contains(outPoint.unsafeFLOSerialize()));
    }
    ...
    public void marriedKeychainBloomFilter() throws Exception {
        ...
        assertFalse(wallet.getBloomFilter(0.001).contains(outPoint.unsafeFLOSerialize()));

        sendMoneyToWallet(BlockChain.NewBlockType.BEST_CHAIN, t1);
        assertTrue(wallet.getBloomFilter(0.001).contains(outPoint.unsafeFLOSerialize()));
    }
    ...
    public void opReturnTwoOutputsTest() throws Exception {
        // Tests sending transaction where one output transfers FLO, the other one writes OP_RETURN.
        receiveATransaction(wallet, myAddress);
        Transaction tx = new Transaction(PARAMS);
        Coin messagePrice = Coin.ZERO;
        Script script = ScriptBuilder.createOpReturnScript("hello world!".getBytes());
    ...
    public void feeSolverAndCoinSelectionTests2() throws Exception {
        ...
        assertTrue(request15.tx.unsafeFLOSerialize().length > 1000);
        ...
        assertTrue(request16.tx.unsafeFLOSerialize().length > 1000);
        ...
        int theoreticalMaxLength17 = request17.tx.unsafeFLOSerialize().length + myKey.getPubKey().length + 75;
        ...
        Transaction spend17 = request17.tx;
        {
            ...
            final int length = spend17.unsafeFLOSerialize().length;
        ...
        int theoreticalMaxLength18 = spend18.unsafeFLOSerialize().length + myKey.getPubKey().length + 75;
        ...
        assertTrue(spend18.unsafeFLOSerialize().length >= 998);
        assertTrue(spend18.unsafeFLOSerialize().length <= 1001);
        ...
        request26.tx.addOutput(CENT.subtract(
                Transaction.REFERENCE_DEFAULT_MIN_TX_FEE.add(Transaction.MIN_NONDUST_OUTPUT).subtract(SATOSHI)),
                OTHER_ADDRESS);
        assertTrue(request26.tx.unsafeFLOSerialize().length > 1000);
    ...
    public void basicCategoryStepTest() throws Exception {
        ...
        request4.feePerKb = Transaction.REFERENCE_DEFAULT_MIN_TX_FEE.divide(request3.tx.unsafeFLOSerialize().length);
        request4.ensureMinRequiredFee = true;
        wallet.completeTx(request4);
    ...
    public void completeTxPartiallySignedWithDummySigs() throws Exception {
        byte[] dummySig = TransactionSignature.dummy().encodeToFLO();
    ...
    public void totalReceivedSent() throws Exception {
        // Receive 4 FLO in 2 separate transactions
        ...
        // Send 3 FLO in a single transaction
