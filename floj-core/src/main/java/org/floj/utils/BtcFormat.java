...
import org.floj.utils.BtcAutoFormat.Style;
...
import static org.floj.utils.BtcAutoFormat.Style.*;
 ...
 * representations of FLO monetary values.
 *
 ...
 * represents a FLO monetary value.  Some of the features offered for doing this
 * are: <ol>
 ...
 * String c = f.format(Coin.COIN);                <strong>// "FLO 1.00"</strong>
 * String k = f.format(Coin.COIN.multiply(1000)); <strong>// "FLO 1,000.00"</strong>
 * String m = f.format(Coin.COIN.divide(1000));   <strong>// "mFLO 1.00"</strong>
 ...
 * code, such as {@code "FLO"}, to indicate the units of denomination, while the
 * {@code SYMBOL} style uses a possibly-ambiguous currency symbol such as
 * {@code "฿"}.
 *
 * <p>The denomination used when formatting will be either flo, millicoin
 ...
 * locale, a value of one flo might be formatted as <pre>฿1.00</pre> where a value
 ...
 * <blockquote><pre>
 * BtcFormat f = BtcFormat.getInstance();
 * String s = f.format(Coin.COIN); <strong>// "FLO 1.00"</strong>
 * </pre></blockquote>
 *
 ...
 * differently the same one-flo value:
 *
 * <blockquote><pre>
 * <strong>// Next line returns "1,00 FLO"</strong>
 ...
 * <h3>Formatting</h3>
 *
 * <p>You format a FLO monetary value by passing it to the {@link BtcFormat#format(Object)}
 * method.  This argument can be either a {@link org.floj.core.Coin}-type object or a
 ...
 * <strong>// next line formats the value as "987.654.321,23 µFLO"</strong>
 * String output = de.format(valueOf(98765432123L), new StringBuffer(), currField);
 * <strong>// next line sets variable currencyCode to "µFLO"</strong>
 ...
 * <strong>// variable coded will be set to "kFLO 1.23"</strong>
 ...
 * {@code µFLO} or other appropriate permutations of those characters.  Additionally, if
...
public abstract class BtcFormat extends Format {
     ...
     * currency code, for example "{@code FLO}", to indicate that denomination.  The
    ...
    /**
     * Return a new auto-denominating instance that will indicate units using a currency
     * code, for example, {@code "FLO"}.  Formatting and parsing will be done
     * according to the default locale.
     */
     ...
     * {@code "FLO"}.  The returned object will format the fraction-part of numbers using
     * the given number of decimal places, or fewer as necessary to avoid giving a place to
     * fractional satoshis.  Formatting and parsing will be done according to the default
     * locale.
     */
     ...
     * units using a currency code, for example, {@code "mFLO"}.
     */
     ...
     * {@code "mFLO"}.  The returned object will format the fraction-part of numbers using
     * the given number of decimal places, or fewer as necessary to avoid giving a place to
     * fractional satoshis.
     */
     ...
     * {@code "฿"}, or code, such as {@code "mFLO"}, depending on the value of
     ...
     * {@code "mFLO"}, depending on the value of the first argument.  The returned object
     ...
     * {@code "FLO"}, or symbol, such as {@code "฿"}, depending on the value
     ...
     * such as {@code "mFLO"}, or symbol, such as {@code "฿"},
    ...
    /**
     * Formats a flo monetary value and returns an {@link java.text.AttributedCharacterIterator}.
    ...
    /**
     * Formats a flo value as a number and possibly a units indicator and appends the
    ...
    /**
     * Formats a flo value as a number and possibly a units indicator to a
    ...
    /**
     * Takes a flo monetary value that the client wants to format and returns the number of
    ...
    /**
     * Takes an object representing a flo quantity of any type the
    ...
    /**
      * Parse a {@link String} representation of a FLO monetary value.  Returns a
      * {@link org.floj.core.Coin} object that represents the parsed value.
      * @see java.text.NumberFormat */
    @Override
    ...
    /** Parse a {@link String} representation of a FLO monetary value.  If this
