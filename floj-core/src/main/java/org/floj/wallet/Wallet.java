...
import org.flo.protocols.payments.Protos.*;
import org.floj.core.AbstractBlockChain;
import org.floj.core.Address;
import org.floj.core.BlockChain;
import org.floj.core.BloomFilter;
import org.floj.core.Coin;
import org.floj.core.Context;
import org.floj.core.ECKey;
import org.floj.core.FilteredBlock;
...
import org.floj.core.PeerFilterProvider;
import org.floj.core.PeerGroup;
import org.floj.core.ScriptException;
import org.floj.core.Sha256Hash;
import org.floj.core.StoredBlock;
import org.floj.core.Transaction;
import org.floj.core.TransactionBag;
import org.floj.core.TransactionBroadcast;
import org.floj.core.TransactionBroadcaster;
import org.floj.core.TransactionConfidence;
import org.floj.core.TransactionInput;
import org.floj.core.TransactionOutPoint;
import org.floj.core.TransactionOutput;
import org.floj.core.UTXO;
import org.floj.core.UTXOProvider;
import org.floj.core.UTXOProviderException;
import org.floj.core.Utils;
import org.floj.core.VarInt;
import org.floj.core.VerificationException;
import org.floj.core.TransactionConfidence.*;
import org.floj.core.listeners.*;
import org.floj.crypto.*;
import org.floj.params.MainNetParams;
import org.floj.script.*;
import org.floj.signers.*;
import org.floj.utils.*;
import org.floj.wallet.Protos.Wallet.*;
import org.floj.wallet.WalletTransaction.*;
import org.floj.wallet.listeners.KeyChainEventListener;
import org.floj.wallet.listeners.ScriptsChangeEventListener;
import org.floj.wallet.listeners.WalletChangeEventListener;
import org.floj.wallet.listeners.WalletCoinsReceivedEventListener;
import org.floj.wallet.listeners.WalletCoinsSentEventListener;
import org.floj.wallet.listeners.WalletEventListener;
import org.floj.wallet.listeners.WalletReorganizeEventListener;
import org.slf4j.*;
 ...
 * {@link Wallet#autosaveToFile(java.io.File, long, java.util.concurrent.TimeUnit, org.floj.wallet.WalletFiles.Listener)}
...
public class Wallet extends BaseTaggableObject
    ...
    // Dead:     Transactions that we believe will never confirm get moved here, out of pending. Note that FLO
     ...
     * {@link org.floj.wallet.KeyChain.KeyPurpose#RECEIVE_FUNDS}. The returned key is stable until
    ...
    /**
     * An alias for calling {@link #currentKey(org.floj.wallet.KeyChain.KeyPurpose)} with
     * {@link org.floj.wallet.KeyChain.KeyPurpose#RECEIVE_FUNDS} as the parameter.
     */
    ...
    /**
     * Returns address for a {@link #currentKey(org.floj.wallet.KeyChain.KeyPurpose)}
     */
    ...
    /**
     * An alias for calling {@link #currentAddress(org.floj.wallet.KeyChain.KeyPurpose)} with
     * {@link org.floj.wallet.KeyChain.KeyPurpose#RECEIVE_FUNDS} as the parameter.
     */
     ...
     * {@link org.floj.wallet.DeterministicKeyChain}. When the parameter is
     * {@link org.floj.wallet.KeyChain.KeyPurpose#RECEIVE_FUNDS} the returned key is suitable for being put
    ...
    /**
     * An alias for calling {@link #freshKey(org.floj.wallet.KeyChain.KeyPurpose)} with
     * {@link org.floj.wallet.KeyChain.KeyPurpose#RECEIVE_FUNDS} as the parameter.
     */
    ...
    /**
     * Returns address for a {@link #freshKey(org.floj.wallet.KeyChain.KeyPurpose)}
     */
    ...
    /**
     * An alias for calling {@link #freshAddress(org.floj.wallet.KeyChain.KeyPurpose)} with
     * {@link org.floj.wallet.KeyChain.KeyPurpose#RECEIVE_FUNDS} as the parameter.
     */
     ...
     * If {@link Wallet#autosaveToFile(java.io.File, long, java.util.concurrent.TimeUnit, org.floj.wallet.WalletFiles.Listener)}
    ...
    /** See {@link org.floj.wallet.DeterministicKeyChain#setLookaheadSize(int)} for more info on this. */
    ...
    /** See {@link org.floj.wallet.DeterministicKeyChain#setLookaheadThreshold(int)} for more info on this. */
     ...
     * See {@link org.floj.wallet.DeterministicKeyChain#markKeyAsUsed(DeterministicKey)} for more info on this.
     */
     ...
     * @throws org.floj.core.ECKey.MissingPrivateKeyException if the seed is unavailable (watching wallet)
     */
    ...
    /**
     * Convenience wrapper around {@link Wallet#encrypt(org.floj.crypto.KeyCrypter,
     ...
     * {@link org.floj.crypto.KeyCrypterScrypt}.
     *
	 ...
	 *
	 * @return signature text for use in verifying or null if address was not found
	 */
     public String signMessage(String addressText, String messageText) {
    	//convert the address from base58
		 Address address = Address.fromBase58(params, addressText);
         //locates a keypair from the basicKeyChain given the hash of the public key. 
	     ECKey key = findKeyFromPubHash(address.getHash160());
	     //if no key found for this address return null
	     if(key==null) return null;
	     //sign the message
	     String signatureBase64 = key.signMessage(messageText);
	     //return the signature
	     return signatureBase64;
     }
    
    /**
     * <p>Method to verify ownership of a FLO receiving address</p>
     * 
     * @param addressText		Base58 Text address used to sign
     * @param messageText		The message that was signed
     * @param signatureText		The signature text from signing
     * @return 					true if message is verified
     * @throws Exception
     */
    public boolean verifyMessage(String addressText, String messageText, String signatureText) {
        //convert the address from base58
        Address address = Address.fromBase58(params, addressText);
        ECKey key;
        //given an arbitrary piece of text and a FLO-format message signature encoded in base64,
        //returns an ECKey containing the public key that was used to sign it
		try {
			key = ECKey.signedMessageToKey(messageText, signatureText);
		} catch (SignatureException e) {
			//could not convert signed message to a key
			return false;
		}
		//returns the address that corresponds to the public part of the created ECKey
        Address gotAddress = key.toAddress(MainNetParams.get());
        //compare the pub address from the key to the pub address being verified
        return (address.equals(gotAddress));
    }
    ...
    private void processTxFromBestChain(Transaction tx, boolean forceAddToPool) throws VerificationException {
        ...
        if (isDeadCoinbase) {
            ...
            // we could. FLO Core nodes on the network have deleted the dependent transactions from their mempools
        ...
        for (TransactionInput input : tx.getInputs()) {
            ...
            if (result == TransactionInput.ConnectionResult.ALREADY_SPENT) {
                if (fromChain) {
                    ...
                    log.warn("{}: {}", tx.getHash(), Utils.HEX.encode(tx.unsafeFLOSerialize()));
                    Transaction other = output.getSpentBy().getParentTransaction();
                    log.warn("{}: {}", other.getHash(), Utils.HEX.encode(other.unsafeFLOSerialize()));
                }
     ...
     * of {@link org.floj.core.ECKey#getCreationTimeSeconds()}. This can return zero if at least one key does
     ...
     * you can use {@link org.floj.utils.Threading#waitForUserCode()} to block until the future had a
    ...
    /**
     * Returns the amount of flo ever received via output. <b>This is not the balance!</b> If an output spends from a
    ...
    /**
     * Returns the amount of flo ever sent via output. If an output is sent to our own wallet, because of change or
    ...
    public static class SendResult {
        /** The FLO transaction message that moves the money. */
     ...
     *
     * @param address The FLO address to send the money to.
     ...
     * @throws ExceededMaxTransactionSize if the resultant transaction is too big for FLO to process.
    ...
    /**
     * Thrown if the resultant transaction is too big for FLO to process. Try breaking up the amounts of value.
     */
    ...
    public void completeTx(SendRequest req) throws InsufficientMoneyException {
        ...
        try {
            ...
            // Check size.
            final int size = req.tx.unsafeFLOSerialize().length;
    ...
    private boolean adjustOutputDownwardsForFee(Transaction tx, CoinSelection coinSelection, Coin feePerKb,
        ...
        final int size = tx.unsafeFLOSerialize().length + estimateBytesForSigning(coinSelection);
            ...
            for (Sha256Hash blockHash : oldBlockHashes) {
                for (TxOffsetPair pair : mapBlockTx.get(blockHash)) {
                    ...
                    if (tx.isCoinBase()) {
                        ...
                        // this coinbase tx. Some can just go pending forever, like the FLO Core. However we
                        // can do our best.
    ...
    public BloomFilter getBloomFilter(int size, double falsePositiveRate, long nTweak) {
        ...
        try {
            ...
            for (TransactionOutPoint point : bloomOutPoints)
                filter.insert(point.unsafeFLOSerialize());
            return filter;
        } finally {
            endBloomFilterCalculation();
        }
    ...
    public FeeCalculation calculateFee(SendRequest req, Coin value, List<TransactionInput> originalInputs,
        ...
        while (true) {
            ...
            // If change is < 0.01 FLO, we will need to have at least minfee to be accepted by the network
            ...
            if (change.signum() > 0) {
                ...
                if (req.ensureMinRequiredFee && changeOutput.isDust()) {
                    ...
                    size += changeOutput.unsafeFLOSerialize().length + VarInt.sizeOf(req.tx.getOutputs().size()) - VarInt.sizeOf(req.tx.getOutputs().size() - 1);
            ...
            size += req.tx.unsafeFLOSerialize().length;
     ...
     * sending transactions to the FLO network by default. Some sendCoins methods let you specify a broadcaster
     ...
     */
    public void setTransactionBroadcaster(@Nullable org.floj.core.TransactionBroadcaster broadcaster) {
     ...
     * @throws org.floj.wallet.DeterministicUpgradeRequiresPassword if key rotation requires the users password.
     */
    ...
    private Transaction rekeyOneBatch(long timeSecs, @Nullable KeyParameter aesKey, List<Transaction> others, boolean sign) {
        ...
        try {
            ...
            checkState(rekeyTx.unsafeFLOSerialize().length < Transaction.MAX_STANDARD_TX_SIZE);
            return rekeyTx;
