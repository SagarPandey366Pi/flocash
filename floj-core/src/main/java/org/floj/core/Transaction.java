...
import org.floj.crypto.TransactionSignature;
import org.floj.script.Script;
import org.floj.script.ScriptBuilder;
import org.floj.script.ScriptOpCodes;
...
import static org.floj.core.Utils.*;
...
public class Transaction extends ChildMessage {
    ...
    /**
     * If feePerKb is lower than this, FLO Core will treat it as if there were no fee.
     */
    public static final Coin REFERENCE_DEFAULT_MIN_TX_FEE = Coin.valueOf(1000); // 0.01 mFLO

    /**
     * If using this feePerKb, transactions will get confirmed within the next couple of blocks.
     * This should be adjusted from time to time. Last adjustment: February 2017.
     */
    public static final Coin DEFAULT_TX_FEE = Coin.valueOf(100000); // 1 mFLO
     ...
     * Max initial size of inputs and outputs ArrayList.
     */    
    public static final int  MAX_FLO_DATA_SIZE = 1040;

    // These are flo serialized.
	 ...
	 *
	 *  StringBuffer(winner) - thread safe, not immutable (faster than String) 
	 *  String       - immutable (slower)
	 *  StringBuilder- no thread safe, not immutable (faster than StringBuffer)
     * 
     */
    private byte[] floData; 
    ...
    public Transaction(NetworkParameters params) {
        ...
        floData = new byte[]{};  //empty byte array to start
    }
    ...
    public Sha256Hash getHash() {
        if (hash == null) {
            hash = Sha256Hash.wrapReversed(Sha256Hash.hashTwice(unsafeFLOSerialize()));
        }
        return hash;
    }

    /**
     * Used by FLOSerializer.  The serializer has to calculate a hash for checksumming so to
     ...
     * value is determined by a formula that all implementations of FLO share. In 2011 the value of a coinbase
    ...
    public String toString(@Nullable AbstractBlockChain chain) {
        ...
        if (floData.length>0) {
            s.append("  floData: ");
            s.append(new String(floData)); //convert to string first to retain actual byte values in string
            s.append('\n');
        }
        ...
        if (fee != null) {
            final int size = unsafeFLOSerialize().length;
    ...
    public void clearInputs() {
        ...
        this.length = this.unsafeFLOSerialize().length;
    }
     ...
     * and requires external synchronization. Please refer to general documentation on FLO scripting and contracts
    ...
    /**
     * Same as {@link #addSignedInput(TransactionOutPoint, org.floj.script.Script, ECKey, org.floj.core.Transaction.SigHash, boolean)}
     ...
     * a wrapper around calling {@link Transaction#hashForSignature(int, byte[], org.floj.core.Transaction.SigHash, boolean)}
    ...
    public Sha256Hash hashForSignature(int inputIndex, byte[] connectedScript, byte sigHashType) {
        ...
        try {
            ...
            // Set the input to the script of its output. FLO Core does this but the step has no obvious purpose as
            ...
            if ((sigHashType & 0x1f) == SigHash.NONE.value) {
                ...
                if (inputIndex >= tx.outputs.size()) {
                    ...
                    // FLO implementation. FLO Core also contains a bug in handling this case:
                    // any transaction output that is signed in this case will result in both the signed output
                    // and any future outputs to this public key being steal-able by anyone who has
                    // the resulting signature and the public key (both of which are part of the signed tx input).

                    // FLO Core's bug is that SignatureHash was supposed to return a hash and on this codepath it
     ...
     * since FLO 0.8+ a transaction that did not end its lock period (non final) is considered to be non
    ...
    /**
     * Returns the confidence object for this transaction from the {@link org.floj.core.TxConfidenceTable}
    ...
    public void verify() throws VerificationException {
        ...
        //check that flo data in transaction is not too large
        if (floData.length > MAX_FLO_DATA_SIZE) {
        	throw new VerificationException.ExcessiveValue();
        }
    }
     ...
     * (<a href="https://github.com/flo/bips/blob/master/bip-0068.mediawiki">BIP 68</a>) if it is version 2 or
     ...
     * <a href="https://github.com/flo/bips/blob/master/bip-0125.mediawiki">full replace-by-fee </a> semantics.
     */
