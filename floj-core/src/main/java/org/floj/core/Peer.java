...
import org.floj.net.StreamConnection;
import org.floj.store.BlockStore;
import org.floj.store.BlockStoreException;
import org.floj.utils.ListenerRegistration;
...
/**
 * <p>A Peer handles the high level communication with a FLO node, extending a {@link PeerSocketHandler} which
 * handles low-level message (de)serialization.</p>
 *
 * <p>Note that timeouts are handled by the extended
 * {@link org.floj.net.AbstractTimeoutHandler} and timeout is automatically disabled (using
 * {@link org.floj.net.AbstractTimeoutHandler#setTimeoutEnabled(boolean)}) once the version
...
public class Peer extends PeerSocketHandler {
     ...
     * {@link org.floj.net.NioClientManager#openConnection(java.net.SocketAddress, StreamConnection)}
     * or
     * {@link org.floj.net.NioClient#NioClient(java.net.SocketAddress, StreamConnection, int)}.</p>
     *
    ...
    /**
     * <p>Construct a peer that reads/writes from the given block chain. Transactions stored in a {@link org.floj.core.TxConfidenceTable}
        ...
        try {
            ...
            try {
                if (awaitingFreshFilter != null) {
                    ...
                    // range of blocks and get an inv both times, due to the codepath in FLO Core hitting
            ...
            if (blocks.size() > 0 && downloadData && blockChain != null) {
                ...
                for (InventoryItem item : blocks) {
                    if (blockChain.isOrphan(item.hash) && downloadBlockBodies) {
                        ...
                        //
                        // Note that as of June 2012 FLO Core won't actually ever interleave blocks pushed as
                        // part of chain download with newly announced blocks, so it should always be taken care of by
                        // the duplicate check in blockChainDownloadLocked(). But FLO Core may change in future so
     ...
     * Once the pong is received the value returned by {@link org.floj.core.Peer#getLastPingTime()} is
     * updated.
    ...
    /**
     * Returns the elapsed time of the last ping/pong cycle. If {@link org.floj.core.Peer#ping()} has never
    ...
    /**
     * Returns a moving average of the last N ping/pong cycles. If {@link org.floj.core.Peer#ping()} has never
     ...
     * after the query returns. <b>Most peers do not support this request. You will need to connect to FLO XT
     ...
     * <b>Most peers do not support this request. You will need to connect to FLO XT peers if you want
     * this to work.</b>
     *
    ...
    public ListenableFuture<UTXOsMessage> getUTXOs(List<TransactionOutPoint> outPoints, boolean includeMempool) {
        ...
        try {
            ...
            if ((peerVer.localServices & GetUTXOsMessage.SERVICE_FLAGS_REQUIRED) != GetUTXOsMessage.SERVICE_FLAGS_REQUIRED)
                throw new ProtocolException("Peer does not support getutxos protocol flag: find FLO XT nodes.");
