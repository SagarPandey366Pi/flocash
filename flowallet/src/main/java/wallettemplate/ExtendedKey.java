...
public class ExtendedKey
{
	...
	{
		return new ExtendedKey (new DeterministicPublicKey(master.getPublic (), true), chainCode, depth, parent, sequence);
	}
	...
	private ExtendedKey generateKey (int sequence) throws ValidationException
	{
		try
		{
			...
			else
			{
				...
				return new ExtendedKey (new DeterministicPublicKey (pub, true), r, depth, parent, sequence);
			}
		}
		catch ( NoSuchAlgorithmException e )
		{
	...
	public String serialize (boolean production)
	{
		...
		return ByteUtils.toHexString(out.toByteArray());
	}

	public static ExtendedKey parse (String serialized) throws ValidationException
	{
		byte[] data = ByteUtils.fromHexString(serialized);
		if ( data.length != 78 )
		{
		...
		if (hasPrivate)
		{
			key = new KeyPair (new BigInteger (1, pubOrPriv), true);
		}
		else
		{
			key = new DeterministicPublicKey(pubOrPriv, true);
		}
